...
export default function LocationChandrapurPage({ onOpenBookingWithLocation, onNavigate }: LocationPageProps) {
  ...
  const locationData = LOCATIONS.find((loc) => loc.name === 'Chandrapur') || LOCATIONS[0];

  // Dynamic SEO Injection (Document Title, Meta description & LocalBusiness JSON-LD)
  useEffect(() => {
    // 1. Title Dynamic Optimization
    const originalTitle = document.title;
    document.title = 'Veda Wellness & Spa, Chandrapur | Premium Salon & Massage Retreat';
    ...
    if (metaDesc) {
      ...
      metaDesc.setAttribute(
        'content',
        'Discover the best premium salon, spa therapies, and relaxing massage treatments in Civil Lines, Chandrapur at Veda Wellness & Spa. Book your visit at our Civil Lines sanctuary in Chandrapur.'
      );
    }
    ...
    const ldJson = {
      ...
      '@type': 'DaySpa',
      'name': 'Veda Wellness & Spa - Chandrapur Sanctuary',
      'image': GALLERY_IMAGES[0],
      'description': 'Our flagship organic spa and beauty sanctuary in Chandrapur. Offering professional haircuts, bridal salon care, and deep traditional massage sessions.',
      'telephone': locationData.phone,
      'address': {
        '@type': 'PostalAddress',
        'streetAddress': 'Veda Mansion, Civil Lines, Near VIP Guest House',
    ...
    script.id = 'ldjson-chandrapur';
    script.innerHTML = JSON.stringify(ldJson);
    document.head.appendChild(script);

    return () => {
      // Cleanup for dynamic SEO tag changes
      document.title = originalTitle;
      if (metaDesc && originalDesc) {
        metaDesc.setAttribute('content', originalDesc);
      }
      const existingScript = document.getElementById('ldjson-chandrapur');
      if (existingScript) {
    ...
    <div className="pt-28 md:pt-36 pb-16 bg-sand-50 min-h-screen text-left">
      <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-12">
        ...
        <div className="border-b border-sand-200 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl font-light text-sand-950 tracking-tight leading-none">
              Chandrapur <span className="text-sage-800 font-serif">Sanctuary</span>
            </h1>
          ...
          <button
            onClick={() => onOpenBookingWithLocation('Chandrapur')}
            ...
            <div className="relative aspect-[16/10] sm:aspect-[1.7] rounded-3xl overflow-hidden border border-sand-200 shadow-lg bg-sand-100 group">
              <AnimatePresence mode="wait">
                <motion.img
                  ...
                  alt="Veda Chandrapur Sanctuary premium salon interiors and relaxing spa chambers"
              ...
              {GALLERY_IMAGES.map((img, idx) => (
                ...
                >
                  <img
                    referrerPolicy="no-referrer"
                    src={img}
                    alt={`Veda Chandrapur space preview ${idx + 1}`}
          ...
          <div className="lg:col-span-5 flex flex-col justify-between gap-6 bg-white p-6 sm:p-8 rounded-3xl border border-sand-200 shadow-md">
            <div className="space-y-6">
              <p className="text-sm text-sand-800 font-sans font-light leading-relaxed text-justify">
                {locationData.description} Escape into a customized, high-luxury treatment program curated in our private therapeutic salons. Designed around high-arched custom interiors, raw clay acoustics, and calming indoor stone water rivers.
              </p>
              ...
              <div className="p-4 rounded-2xl bg-sand-100 border border-sand-200 flex items-start gap-3 text-xs leading-relaxed text-sand-700">
                <Check className="w-4.5 h-4.5 text-amber-600 shrink-0 mt-0.5" />
                <div>
        ...
        <div className="w-full aspect-[21/9] min-h-[300px] max-h-[420px] rounded-[2rem] overflow-hidden border border-sand-200 shadow-sm">
          <iframe
            title="Route Map for Veda Wellness & Spa, Chandrapur"
            src={MAP_EMBED}
            width="100%"
            height="100%"
        ...
        <section className="bg-white border border-sand-200 rounded-[2rem] p-8 md:p-12 space-y-8">
          <div className="max-w-4xl space-y-4">
            ...
            <h2 className="font-serif text-2xl sm:text-3xl text-sand-950 font-light tracking-tight">
              Best Spa & Premium Hair Salon in Civil Lines, Chandrapur
            </h2>
            <p className="font-sans text-sm text-sand-700 font-light leading-relaxed">
              Veda Wellness & Spa stands as Chandrapur's premier location for professional salon aesthetics, advanced wellness therapies, and world-class massage routines. Located centrally in Civil Lines near the VIP Guest House, we bring highly trained professional hair stylists and elite massage therapists together in a serene, eco-conscious sanctuary. 
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-sand-100 pt-8 text-sm">
            <div className="space-y-3">
              <h3 className="font-serif font-medium text-sand-950">Luxury Massage Services</h3>
              <ul className="space-y-2 text-sand-600 font-light">
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-sage-800 shrink-0" />
                  <span>Deep Tissue, Balinese, and Swedish Therapies</span>
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-sage-800 shrink-0" />
                  <span>Hot Botanical Oils & Ayurvedic Relief Practices</span>
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-sage-800 shrink-0" />
                  <span>Couple massage rooms with custom privacy blocks</span>
                </li>
              </ul>
            </div>

            <div className="space-y-3">
              <h3 className="font-serif font-medium text-sand-950">Premium Hair Salon</h3>
              <ul className="space-y-2 text-sand-600 font-light">
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-sage-800 shrink-0" />
                  <span>Precision Haircuts, global dye, and root touch ups</span>
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-sage-800 shrink-0" />
                  <span>Keratin protein spas, hair botox, and smoothing</span>
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-sage-800 shrink-0" />
                  <span>Facial treatments including O3+ and Hydra facials</span>
                </li>
              </ul>
            </div>
          </div>
